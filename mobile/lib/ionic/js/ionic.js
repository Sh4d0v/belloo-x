...
(function(window, document, ionic) {
  ...
  var self = ionic.Platform = {
    ...
    isIPad: function() {
      ...
      if (self.navigator.platform === 'MacIntel' && self.navigator.maxTouchPoints > 1) {
        return true;
      }
    ...
    setPlatform: function(n) {
      ...
      } else if (/iPhone|iPad|iPod/.test(self.ua) || (self.navigator.platform === 'MacIntel' && self.navigator.maxTouchPoints > 1)) {
        platformName = IOS;
      } else {
        ...
        */
        ...
        if (!alreadyShrunk || changedKeyboardHeight) {
          ...
          if ( !isPopover && (ionic.Platform.isIOS() || ionic.Platform.isFullScreen || isInsetModal) ) {
            ...
            ionic.requestAnimationFrame(function(){
              // D - A or B - A if D > B       D - A             max(0, D - B)
              scrollViewOffsetHeight = Math.max(0, Math.min(self.__originalContainerHeight, self.__originalContainerHeight - (e.detail.keyboardHeight - (window.innerHeight - rect.bottom))));//keyboardOffset >= 0 ? scrollViewOffsetHeight - keyboardOffset : scrollViewOffsetHeight + keyboardOffset;
